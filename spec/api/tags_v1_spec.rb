  ...
  describe :suggested do 
    ...
    context "when logged in" do 
      ...
      it "provides user's tags" do 
        post "tags/suggested", {:ignore_user => false, :selected_tags => ['foo']}, {"Accept-Version" => "v1"}
      ...
      it "ignores users's tag when asked" do 
        post "tags/suggested", {:ignore_user => true, :selected_tags => ['foo']}, {"Accept-Version" => "v1"}
        h = JSON.parse(response.body)
        h["user_tags"].blank?.should be_true
      end

      it "provides recommended tags" do 
        post "tags/suggested", {:selected_tags => ['foo']}, {"Accept-Version" => "v1"}
        h = JSON.parse(response.body)
